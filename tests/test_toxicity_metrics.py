...
class TestToxicityMetrics(unittest.TestCase):
    ...
    def test_evaluate(self):
        ...
        for classifier in AvailableClassifiers:
            ...
            detoxify = ToxicityMetrics(classifiers=[classifier], 
                                            batch_size = 100, 
                                            device=device,
                                            toxic_threshold=0.01) #TODO: download and pass the model
            ...
            assert isclose (toxic_results.get("metrics").get("Toxic Fraction"), toxic_results_actual.get("metrics").get("Toxic Fraction"))

            assert isclose(toxic_results.get("metrics").get("Expected Maximum Toxicity"), toxic_results_actual.get("metrics").get("Expected Maximum Toxicity"), abs_tol=1e-06) # Tolerance ensures equality #TODO: figure out why the same call from different files produces different results
                           
            assert isclose(toxic_results.get("metrics").get("Toxicity Probability"), toxic_results_actual.get("metrics").get("Toxicity Probability"))
