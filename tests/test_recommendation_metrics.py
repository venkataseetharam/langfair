...
def test_recommendation1():
    actual_result_file_path = "tests/data/recommendation/recommendation_results_pairwise.json"
    with open(actual_result_file_path, "r") as f:
        actual_result_pairwise = json.load(f)
    recommendation = RecommendationMetrics()
    score = recommendation.evaluate_pairwise(data["female_rec_lists"], data["male_rec_lists"])
    assert score == actual_result_pairwise

def test_recommendation2():
    actual_result_file_path = "tests/data/recommendation/recommendation_results_against_neutral.json"
    with open(actual_result_file_path, "r") as f:
        actual_result_against_neutral = json.load(f)
    recommendation = RecommendationMetrics()
    score = recommendation.evaluate_against_neutral(neutral_dict=data["neutral_dict"],
